...
class DBSelect extends DB {
	...
	private void printUser(String sql) throws SQLException{
		...
		//ResultSet의 개수를 구하는 메서드는 없어서 다른 방법으로 개수 구함
		...
		if(totalRow ==1) {
			...
			//전체 목록 출력
			//데이터의 처음부터 읽어오므로 next를 만나면 한 줄로 읽어오고, 다음 줄로 커서가 이동
			//한줄 한줄 내려가면서 데이터를 가져오고, 데이터가 끝까지 감녀 종료
	...
	public void getOneUser() throws SQLException {
		...
		do { 
			...
			//입력값이 y가 아닌 경우 true, 같은 경우 false 반환
