...
class DBCreate extends DB {
	private void createTable (String tableName, String sql) {
		...
		try {
			...
			//아리 대 항목은 부모 클래스에서 저으이
			//sql문을 전송 할 수 있는 PreaparedStatement 객체를 생성
			preparedStatement = conn.prepareStatement(tableSql); //쿼리 실행 준비
			resultSet = preparedStatement.executeQuery(); //쿼리 실행 후 결과 받음
			while(resultSet.next()) {
				if(tableName.equals(resultSet.getString(1))) { //동일한 테이블이 있는경우
					isTable = true;
				}
	...
	public void createTableUser() {
		...
		String sql = "CREATE TABLE " + tableName + "(userID VARCHAR(100) PRIMARY KEY, " + "name VARCHAR(100), age INT, job VARCHAR(100))";
